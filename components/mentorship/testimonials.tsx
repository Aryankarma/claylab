    ...
    <section className="py-16 px-4 ">
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {testimonials.map((t, i) => (
          <div
            key={i}
            className={`${t.bgColor} rounded-md shadow-[10px_10px_20px_rgba(0,0,0,0.3)] p-12 min-h-[300px] flex flex-col justify-between`}
          >
            <p className="text-xl font-medium text-right  leading-snug font-roughScript">{t.name}</p>
            <p className="text-3xl md:text-4xl text-center font-medium mt-4 font-roughScript">
              {t.quote}
            </p>
          </div>
        ))}
